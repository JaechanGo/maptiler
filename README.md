...
## 점군(PCD) 내보내기 — 지형·시설물 확인용

데모(`/demo/`)의 **PCD 내보내기** 버튼이 화면 중심 기준 정사각형 범위를 PCL `.pcd` 점군으로 뽑는다.
CloudCompare·PCL·Open3D 에서 열어 지형 기복과 건물 배치를 3D로 확인하는 용도다.

- **지형** — Terrain-RGB 타일(`/data/terrain`, z12)을 브라우저에서 직접 디코드해 이중선형 보간.
  **원본 DEM 해상도가 30m(SRTM)** 이므로 격자를 그보다 촘촘히 잡아도 정보량은 늘지 않는다(기본 10m).
- **건물** — `buildings` 소스(martin `/dyn/building`)의 폴리곤 + `render_height` 를 지붕 격자 + 벽면
  수직 샘플로 압출(기본 1m). 지반고도는 DEM 에서 가져와 지형 위에 얹는다.
- **도로·철도** — `openmaptiles`/`transportation` 중심선을 class 별 대표 실폭(motorway 24m … path 2m,
  rail 5m)으로 펴서 지형 위 0.15m 에 얹는다. 벡터타일에 폭 속성이 없어 폭은 class 로 준다.
  터널(`brunnel=tunnel`)은 지표 판독을 방해하므로 뺀다.
- **물·녹지** — `water`/`park`/`landcover` 폴리곤 안에 드는 **지형점의 색만** 바꾼다.
  점을 새로 만들지 않으므로 용량이 늘지 않는다.
- **미리보기·속도** — 패널의 **미리보기** 는 같은 샘플러를 성긴 간격(지형 20m·건물 4m, 점 수 ≈1/16)으로 돌려 1~2초 안에
  평면도(위에서)·등각 투영(남서→북동)을 그리고, 내보낸 뒤엔 실제 점군을 그대로 그린다. 샘플링은 Web Worker 에서 돌고
  지붕은 스캔라인 채우기라 500m·건물 1m 기준 수 초(이전 35초)이며 생성 중에도 지도 조작이 멈추지 않는다.
- **좌표** — 로컬 ENU 미터(x=동, y=북, z=해발). 원점(화면 중심) 경위도는 헤더 주석과 파일명에 기록된다.
  경위도 그대로나 UTM-K 절대좌표는 float32 에서 ~1.5cm 로 양자화되므로 쓰지 않는다.
- **필드** — `x y z rgb`(rgb = `0x00RRGGBB` 를 float32 로 재해석하는 PCL 관행). 색상 해제 시 `x y z`.
  지표=하이프소메트릭 · 수역=청 · 녹지=녹 · 건물=난색(높이) · 도로=주황/노랑/회색 · 철도=보라로
  계열을 나누고, 계열 안에서 고도로 명암이 지므로 분류와 기복이 동시에 읽힌다.

> 현재는 브라우저 전용(서버 무변경)이라 한계가 있다: martin 이 `clip_geom: true` 라 **타일 경계에서
> 건물이 잘려 내부에 가짜 벽면**이 생기고, **화면에 로드된 타일**의 건물만 나오며, 대면적은 메모리에 막힌다.
> 원본 폴리곤을 그대로 쓰는 서버측 내보내기(`/pcd` 엔드포인트)로 옮기면 해소된다 —
> 그때도 위 헤더·좌표축 계약은 그대로 유지한다(`demo/js/pcd-export.js` 머리말 참조).
