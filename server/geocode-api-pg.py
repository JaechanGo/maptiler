...
def geocode(cur, q, limit, meta=None):
    ...
    if not results and p["dong"] and p["house"]:
        ...
        if cds:
            ...
            # 정확 필지가 없으면 **같은 본번**의 다른 부번을 제안한다. **본번 근사는 금지** —
            # 없는 본번(S2)에 근사를 걸면 전혀 다른 필지를 정답처럼 내놓는다(빈 결과가 정답이다).
            # 그 금지는 위 WHERE 의 `ji_main = %s` 가 담보하므로 부번 조건과 무관하다. 종전의
            # `h[1]` 게이트는 부번 미명시('아산리 458' → (458,0)) 질의를 근사에서 배제했는데,
            # 원필지(-0)가 없고 458-2·458-3 만 실재하는 지번에서 전면 0건이 됐다(NO304, VWorld
            # 는 동일 질의에 인근 부번을 반환). 부번 미명시도 근사를 연다 — 정렬식 abs(ji_sub-0)
            # 은 '가장 낮은 부번부터'로 자연 일반화된다.
