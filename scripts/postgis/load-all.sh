...
if has admin; then
  ...
  [ -d "$LEGAL" ]     && run "$HERE/load_admin.sh" --shp "$LEGAL" --level emd --srs EPSG:5186 --name-field EMD_NM --code-field EMD_CD \
                      || echo "  (건너뜀) 법정동 경계 없음: $LEGAL"
  [ -f "$ADMIN_SHP" ] && run "$HERE/load_admin.sh" --shp "$ADMIN_SHP" --level adm_dong --srs EPSG:5186 --name-field ADM_NM --code-field ADM_CD \
                      || echo "  (건너뜀) 행정동 경계 없음: $ADMIN_SHP"
...
if has parcel; then
  ...
  [ -d "$PARCEL" ] && run "$HERE/load_parcel.sh" --shp "$PARCEL" --fresh \
...
if has geocode; then
  ...
  [ -f "$GDB" ] && run python3 "$HERE/load_geocode.py" --db "$GDB" \
