-- ⚠⚠ 폐기 (T018 Phase 1, 2026-08-10). 실행 불가 — 아래 가드가 즉시 중단시킨다. ⚠⚠
--
--   이 스크립트는 리 사전을 **address(도로명주소 건물 DB)에서 역산**한다. 원본을 안 읽으므로
--   건물 없는 리가 통째로 빠지고(16,113 / 법정리 약 19,000), 09-gen-geocode.py:175 의
--   setdefault 와 맞물려 같은 읍면의 리 이름이 하나만 살아남아 코드가 뭉개진다.
--   → 대체: scripts/postgis/build_ri_dict_from_lawd_code.sql (행안부 법정동코드 원본 기반)
--     선행: scripts/postgis/load_lawd_code.py (원본 → lawd_code 53,387행)
--     신 기대값: 총 17,896 / 폐지 폴백 2,687 / 폴백 시도 46 단일 / 존재티어 유일성 100%
--
--   ※ 아래 lo=15000 / hi=20000 범위 가드는 신 사전 17,896 을 **무경고 통과**시킨다.
--     즉 이 스크립트를 잘못 재실행하면 사전이 조용히 구 방식으로 되돌아간다. 그래서
--     범위 조정이 아니라 실행 자체를 막는다.
--
--   폐기 이력·한계 서술은 아래 원문에 그대로 남겨 둔다(왜 이 축이 선택됐고 왜 부족했는지의 기록).
--   갱신 절차는 docs/ri-dict-runbook.md 를 볼 것.
