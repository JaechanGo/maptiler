-- ############################################################################
-- ##  ★ DEPRECATED — 생성 방식(명칭 꼬리 조인)은 신뢰할 수 없다  (T026, 2026-08-18)
-- ##
-- ##  이 스크립트는 신구 법정동코드 대응을 **법정동명의 꼬리 문자열 조인**으로 추론한다.
-- ##
-- ##  아래 '매핑 규칙' 주석이 기록한 수치를 정확히 읽을 것 —
-- ##      양쪽이 모두 성립하는 402건 중 결론 일치는 244건(60.70%)뿐.
-- ##  이것은 **이 스크립트의 정확도가 아니라**, 명칭 기반(가설b)과 "시도 2자리만 치환"
-- ##  (가설a)의 **결론이 서로 갈린 비율**이다. 원문은 이 값을 가설a 배제 근거로 썼고,
-- ##  어느 쪽이 옳은지는 판정하지 못했다. **그 판정을 원천이 대신해 준 것이 T026 이다.**
-- ##
-- ##  deprecated 사유는 정확도 수치가 아니라 **방법의 구조적 한계**다:
-- ##      · 동명은 전국에서 중복된다(서울 중구·대구 동구·부산 서구…).
-- ##      · 한 시도 안에서도 2:2 로 겹치는 사례가 실재한다(인천 신 금곡동).
-- ##        2812510600 제물포구 금곡동 ← 2814010600 / 2829010700 검단구 금곡동 ← 2826011800
-- ##  즉 **명칭만으로는 대응이 결정되지 않는다.** 조인 조건을 다듬어 고칠 수 있는 문제가
-- ##  아니다. 원천이 대응을 직접 주는데 추론으로 대체할 이유도 없다.
-- ##
-- ##  ── 대체 축 ──────────────────────────────────────────────────────────────
-- ##  VWorld 법정동코드 데이터셋 **dsId=30505 의 `OLD_LAWDCD` 컬럼**을 쓴다.
-- ##  신 코드 행이 자기 옛 코드를 **스스로 가리키므로 추론이 0** 이다.
-- ##      본보기: scripts/postgis/build_incheon_remap_from_old_lawdcd.sql
-- ##      원천 적재: scripts/load_lawd_code_v2.py  (→ lawd_code_v2)
-- ##
-- ##  ── ★ 그러나 테이블은 지우지 말 것 ──────────────────────────────────────
-- ##  이 스크립트가 만든 **lawd_sido_remap 테이블은 T018 안 A 가 현재도 사용 중**이다.
-- ##  지우면 전남·광주 응답 치환이 즉시 죽는다(fail-open 으로 46/29 가 그대로 노출된다).
-- ##  정본 650쌍과 대조한 결과 수록 범위 안에서는 일치 623 · 불일치 0 이었다 — **표 자체에는
-- ##  문제가 없다.** deprecated 인 것은 **표가 아니라 생성 방식(명칭 조인)** 이다.
-- ##  같은 이유로 lawd_ri_remap_exception 도 남긴다(_load_sido_remap 이 두 표를
-- ##  all-or-nothing 으로 검사하므로 한쪽만 지우면 치환 전체가 꺼진다).
-- ##
-- ##  ── 재생성이 필요하다면 ──────────────────────────────────────────────────
-- ##  이 스크립트를 다시 돌리지 말고, **OLD_LAWDCD 축으로 새로 만들어라.**
-- ##  명칭 조인 SQL 을 새로 작성하는 것은 금지다(T021 이 그 방식으로 실패했다).
-- ##
-- ##  ── 참조 ─────────────────────────────────────────────────────────────────
-- ##      docs/incheon-sgg-remap.md   §2(왜 OLD_LAWDCD 인가) · §12(이 표의 처분 판단)
-- ############################################################################
--
-- ============================================================================
