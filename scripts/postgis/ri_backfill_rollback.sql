-- [F1 처분 2026-08-28 · T045] ri 백필 파이프라인은 폐기됐다(사유: ri_backfill_s3_todo.sql 헤더).
--   이 롤백 스크립트만은 실행 가능 상태로 남긴다 — 과거에 백필이 적용된 환경을 되돌릴 때
--   필요하다. 새 백필 실행은 금지다(s3_todo·s5_apply 는 가드로 차단됨).
